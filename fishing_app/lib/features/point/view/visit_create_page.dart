...
class _VisitCreatePageState extends ConsumerState<VisitCreatePage> {
  ...
  void _usePointLocation(FishingPoint point) {
    if (point.latitude == null || point.longitude == null) return;
    setState(() {
      _lat = point.latitude;
      _lon = point.longitude;
      _addressCtrl.text = point.address?.isNotEmpty == true ? point.address! : point.name;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final asyncPoint = ref.watch(pointDetailProvider(widget.pointId));
    final dateStr =
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('낚시한 위치', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold)),
                const Text(
                  '같은 포인트라도 실제로 낚시한 자리는 방문마다 다를 수 있어 따로 지정할 수 있어요.',
                ...
                Wrap(
                  spacing: 4,
                  children: [
                    asyncPoint.maybeWhen(
                      data: (point) => (point.latitude != null && point.longitude != null)
                          ? TextButton.icon(
                              onPressed: () => _usePointLocation(point),
                              icon: const Icon(Icons.push_pin_outlined, size: 18),
                              label: const Text('포인트 위치 사용'),
                            )
